            ...
            <form method="post" action="/dashboard/posts/{{ $post->slug }}" enctype="multipart/form-data"
                ...
                <div class="mb-4 border-b border-gray-200 dark:border-gray-700">
                    <ul class="-mb-px flex flex-wrap text-center text-sm font-medium" id="default-tab">
                        <li class="me-2">
                            <button x-on:click="activeTab = 'tabBasic'"
                                :class="activeTab === 'tabBasic' ? 'border-blue-500 text-blue-600' : ''"
                                class="inline-block rounded-t-lg border-b-2 p-4" id="basic-tab"
                                type="button">Basic</button>
                        </li>
                        <li class="me-2">
                            <button x-on:click="activeTab = 'tabAdvance'"
                                :class="activeTab === 'tabAdvance' ? 'border-blue-500 text-blue-600' : ''"
                                class="inline-block rounded-t-lg border-b-2 p-4" id="advance-tab"
                ...
                <div id="default-tab-content">
                    {{-- Tab 1 --}}
                    <div x-show="activeTab === 'tabBasic'" x-cloak
                        class="rounded-lg border-b bg-gray-50 p-4 dark:bg-gray-800" id="basic">
                        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                            <div class="sm:col-span-2">
                                ...
                                <input type="text" name="title" id="title"
                                    ...
                                    placeholder="Type title" autofocus value="{{ old('title', $post->title) }}">
                                @error('title')
                            ...
                            <div class="w-full">
                                ...
                                <input type="text" name="slug-dummy" id="slug"
                                    class="{{ $errors->has('slug')
                                        ? 'border-pink-500 text-pink-600 focus:border-pink-500 focus:ring-pink-500'
                                        : 'border-gray-300 text-gray-900 focus:border-primary-600 focus:ring-primary-600' }} block w-full rounded-lg border bg-gray-300 p-2.5 text-sm shadow-inner hover:cursor-not-allowed dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-primary-500 dark:focus:ring-primary-500"
                                    placeholder="Slug" value='{{ $post->slug }}' disabled>
                            ...
                            <div class="w-full">
                                ...
                                <div class="relative">
                                    <input type="text" name="published_at" id="published_at"
                                        ...
                                        placeholder="Input date" value="{{ old('published_at', $post->published_at) }}">
                            ...
                            <div x-data="{ categories: {{ $category }}, selectedCategory: '{{ old('category_id') ?? $post->category_id }}' }">
                    ...
                    {{-- Tab 2 --}}
                    <div x-show="activeTab==='tabAdvance'" x-cloak
                        class="rounded-lg border-b bg-gray-50 p-4 dark:bg-gray-800" id="advance">
                        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                            <div class="sm:col-span-2">
                                <label for="metaTitle"
                                ...
                                <input type="text" name="metaTitle" id="metaTitle"
                                    class="{{ $errors->has('metaTitle')
                                        ? 'border-pink-500 text-pink-600 focus:border-pink-500 focus:ring-pink-500'
                                        : 'border-gray-300 text-gray-900 focus:border-primary-600 focus:ring-primary-600' }} block w-full rounded-lg border bg-gray-50 p-2.5 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-primary-500 dark:focus:ring-primary-500"
                                    placeholder="Type metaTitle" autofocus
                            ...
                            <div class="sm:col-span-1" x-data="{
                                ...
                                <input type="hidden" name="parent_id" id="parent_id"
                                    x-bind:value="selectedPost.id ? selectedPost.id : '{{ old('parent_id', $post->parent_id) }}'">
                                <div class="group relative inline-block w-full">
                                    <button type="button"
                                        ...
                                        <div x-text="selectedPost ? selectedPost.title : '{{ old('parent_id')
                                            ? (($parentPost = $posts->where('id', old('parent_id'))->first())
                                                ? $parentPost->title
                                                : 'Select Related post')
                                            : (($parentPost = $posts->where('id', $post->parent_id)->first())
                                                ? $parentPost->title
                                                : 'Select Related post') }}'"
                                            x-on:click="open = !open"
                                            class="h-full w-full cursor-pointer truncate whitespace-nowrap p-2.5 text-left text-gray-500">
                                        </div>
                                            ...
                                            <div class="relative h-full w-full">
                                                <template x-for="post in posts" :key="post.id">
                                                    <li x-show="post.title.toLowerCase().includes(searchQuery.toLowerCase())"
                                                        ...
                                                        class="w-full cursor-pointer py-1 pl-2 text-left text-gray-400 hover:bg-gray-200">
                                                    </li>
                                                </template>
                                            </div>
                                        </ul>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea type="text" name="summary" id="summary"
                                    ...
                                    placeholder="Type summary">{{ old('summary', $post->summary) }}</textarea>
                                @error('summary')
                                    <div class="text-pink-600">
                                        {{ $message }}
                                    </div>
