<form class="mb-6" x-data="{ comment: {{ $comment }}, other: '' }" x-show="reply || editMode" x-cloak
    @click.outside="reply = false; editMode = false">
    @csrf
    <div
        ...
        <textarea id="content" rows="6" wire:model="content"
            ...
            placeholder="Write a comment..." {{ Auth::check() ? '' : 'disabled' }} x-model="editMode ? comment.content : other"></textarea>
    </div>
    @error('content')
    ...
    <div class="flex w-full justify-end gap-6">
        ...
        <button type="button" x-on:click="reply = false; editMode = false"
            class="inline-flex cursor-pointer items-center rounded-lg bg-gray-400 px-4 py-2.5 text-center text-xs font-medium text-white hover:bg-gray-500 focus:ring-4 focus:ring-primary-200 dark:focus:ring-primary-900">
            Cancel
        </button>
        <button type="button" x-on:click="editMode ? $wire.update : $wire.save" wire:loading.attr="disabled"
            wire:target="content" wire:loading.class="hover:cursor-not-allowed"
